...
namespace LuaGameObject
{
    int GetRelativePoint(lua_State* L, GameObject* go)
    {
        ...
        int deg = sEluna.CHECKVAL<int>(L, 2);
    ...
    int SummonCreature(lua_State* L, GameObject* go)
    {
        ...
        uint32 spawnType = sEluna.CHECKVAL<uint32>(L, 6, 0);
    ...
    int RemoveFromWorld(lua_State* L, GameObject* go)
    {
        ...
        bool del = sEluna.CHECKVAL<bool>(L, 1, false);
        if (del)
            go->DeleteFromDB();
        go->RemoveFromWorld();
        return 0;
    ...
    int UseDoorOrButton(lua_State* L, GameObject* go)
    {
        ...
        uint32 delay = sEluna.CHECKVAL<uint32>(L, 1, 0);
    ...
    int SetGoState(lua_State* L, GameObject* go)
    {
        ...
        uint32 state = sEluna.CHECKVAL<uint32>(L, 1, 0);
    ...
    {
        uint32 delay = sEluna.CHECKVAL<uint32>(L, 1, 1);
        if (!delay)
            delay = 1;
