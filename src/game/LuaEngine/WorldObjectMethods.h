...
namespace LuaWorldObject
{
    ...
    {
        bool nearest = sEluna.CHECKVAL<bool>(L, 1, true);
        float range = sEluna.CHECKVAL<float>(L, 2, SIZE_OF_GRIDS);
        uint16 type = sEluna.CHECKVAL<uint16>(L, 3, 0); // TypeMask
        uint32 entry = sEluna.CHECKVAL<uint32>(L, 4, 0);
        uint32 hostile = sEluna.CHECKVAL<uint32>(L, 5, 0); // 0 none, 1 hostile, 2 friendly
    ...
    {
        WorldObject* target = sEluna.CHECKOBJ<WorldObject>(L, 1);
